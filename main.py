...
def main(stop_event, time_test, model_name, display_queue, faceRec_queue, post_queue):
...
append_string_to_csv("license Code Recognition system has been shut down.", 'log.csv')
                ...
                #                         if person.face.faceProposal.img.size>8000:  #NOTE:!!!IMPORTANT PARAMETER: adjust min & max face size from here!!!
                #                             cv2.imshow("face_cropped", person.face.faceProposal.img)
                #                             cv2.waitKey(1)
                #                             if not person.face.isFaceIdentifiedProperly:
                #                                 #log write
                ...
                #                                     info = result_person_info(person)
                #                                     append_item_to_json(trackerId, person, "db_json")
                #                                     # info.send_identified_person_info_to_db()
                #                                 elif person.face.face_finalizer[0] == person.face.face_finalizer[1] == person.face.face_finalizer[2] == "Unknown":
                #                                     person.face.face_finalizer = ["", "", ""]
                #                                     person.face.unknown_count+=1
                #                                     if person.face.unknown_count >= 10:
                #                                         print(f'{person.face.faceProposal.name}    NEW UNKNOWN FACE IS FOUND!!!')
...
#             defaultFrame = frame.copy()
#             #motion detection algorithm
#             if not motion_detector.motion_detected and previous_frame is not None:
#                 st_MD = time.time()
#                 motion_detector.motion_checker(previous_frame, defaultFrame)
#                 et_MD = time.time()
#                 if time_test == 1:
...
#                                         if person.face.faceProposal.img.size>8000:  #!!!IMPORTANT PARAMETER: adjust min & max face size from here!!!
#                                             cv2.imshow("face_cropped", person.face.faceProposal.img)
#                                             cv2.waitKey(1)
#                                             #log write
#                                             append_string_to_csv(f"Face is detected. tracker_id: {trackerId}", 'log.csv')
